...
public static class TypeChart
{
    ...
    //2.0 means supereffective, 0.5 means not very effective, 0.0 means immune
    //all matchups excluded from dictionary are neutral (1.0)
    private static readonly Dictionary<(PokemonType Attack, PokemonType Defense), double> Effectiveness = new()
    {
        //normal:
    ...
    public static double getEffectiveness(PokemonType attacking, PokemonType defending)
    {
        if(Effectiveness.TryGetValue((attacking, defending), out double multiplier)) //if the macthup is non neutral, it will be in the hashmap
        {
            return multiplier;
        }

        return 1.0; //if it's not in the hashmap, it's neutral
    }
}